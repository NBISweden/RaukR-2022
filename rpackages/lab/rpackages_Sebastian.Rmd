...
> Note: The information can look slightly different depending on how the package was created.
...
<!-- Does this part need devtools::build() or can one test melt() using for example load_all()? Otherwise explain what build() does. --> 
...
> Note: An alternative way of achieving the behaviour of a correct NAMESPACE is writing your code with strict package references. In our case `reshape2::melt()`. This way you can circumvent using `roxygen2` to add import tags to `NAMESPACE` if you want. I recommend doing both =).
...
> Note: To be able to use the functions of your package in the vignette you will need to install your package, the easiest way is to execute `devtools::install()`.
...
> Note: Some users have reported receiving some warnings when executing `usethis::use_rcpp()` but it didn't seem to break anything.
